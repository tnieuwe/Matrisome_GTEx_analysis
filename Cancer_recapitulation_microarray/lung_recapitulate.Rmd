...
```{r}
top_table_sig_genes <- function(top_table, gene_vect, table_res = FALSE, one_or_zero = FALSE){
    out_dat <- top_table %>% filter(Gene.symbol %in% gene_vect) %>%
        select(ID, Gene.symbol, logFC, AveExpr, t, P.Value, P.Value, adj.P.Val)
    if (table_res == FALSE) {
        return(out_dat)
...
top_table_sig_genes(tT2, top_genes, table_res = T,one_or_zero = T)


top_table_sig_genes(tT2, bot_genes, table_res = T,one_or_zero = T)
```
