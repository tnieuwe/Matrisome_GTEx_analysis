...
gset <- getGEO("GSE109169", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
# gset <- gset[,ind]
samp_dat <- pData(gset)
sml <- as.numeric(str_detect(samp_dat$title, "Tumor"))
```
...
gset_gene_checker <- function(gset_dat, gene){
    features <- fData(gset_dat)
    gene_list <- str_split(features$gene_assignment, "[/][/]")
    genes_hopefully <- for (gene in gene_list) {
        gene
    }
    ind <-sapply(gene_list, "[[", 2) %in% gene
    sub_gset <- ind
...
gene_vect <- bot_genes
top_table <- tT2
#New top table for just us
top_table_2 <- top_table %>% separate_("gene_assignment", c("gene_id", "Gene.symbol"), sep = "[/][/]") %>%
  mutate(Gene.symbol = str_trim(Gene.symbol, "both"))
top_table <- top_table_2
...
top_sig <- top_table_sig_genes(top_table_2, top_genes, table_res = T,  drop_not_sig = T)
top_sig

bot_sig <- top_table_sig_genes(top_table_2, bot_genes, table_res = T,  drop_not_sig = T)
bot_sig
