...
for (cur_gene in colnames(cor_mat)) {
    ...
    temp_sub = paste0("Pvalue = ", format.pval(test_out$p.value),
                      ...
                      "; Non-CAF mean = ",
                      round(summarized_means[summarized_means$CAF_or_not != "CAF", 2], 4))
    # hold_plt <- ggplot(final_dat_plot, aes_string("CAF_or_not", cur_gene)) +
    #                 geom_sina(alpha = 0.5) +
    hold_plt <- ggplot(final_dat_plot, aes_string(fill = "CAF_or_not", x= cur_gene)) +
                geom_histogram(alpha = 0.5,position = "identity") +
         scale_y_log10() +
        labs(title = paste0(cur_gene, "; ", rank_val),
             subtitle = temp_sub) +
        theme_classic()
