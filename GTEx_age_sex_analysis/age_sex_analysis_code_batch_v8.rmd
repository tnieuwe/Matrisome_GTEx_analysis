...
for (tiss in tiss_list) {
  file_name <- paste0("input/","limma_result_batch_files",
                      "/limma_",tiss,"_res_age_batch.csv")
...
master_lm <- master_lm %>% mutate(all.correct.pval = p.adjust(pvalue, method = "BH"))
...
write.csv(master_lm, file = "output/table_S1_all_age_results_batch.csv")
```
...
ggsave("output/core_reg_top-pval_lm_v8_new_batch.pdf")
```
...
ggplot(beta_lm_all) +
  ...
  scale_fill_manual(name = "Matrix category", values = c("#E66100", "#5D3A9B", "#E1BE6A", "#994F00")) +
...
ggsave("output/fig_2a_top_beta_age_change_batch.pdf", width = 10, height = 7.5)
...
ggsave("output/fig_2a_small_pval_age_change_batch.pdf", width = 10, height = 7.5)
...
sample_dat <- as.data.frame(fread("../global_in/GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt"))

subject_dat <- select(subject_dat, SUBJID, compare_columns, SEX, DTHHRDY)
sample_dat <- mutate(sample_dat, SUBJID = GTEx_SAMPID_to_SUBJID(SAMPID)) %>%
  select(SUBJID, SMTSISCH, SMGEBTCH)

final_frame <- left_join(gene_frame, subject_dat) %>% left_join(sample_dat)

final_frame <- final_frame %>%
  mutate(SUBJID = str_remove(string = SUBJID, "GTEX-")) %>% select(SUBJID, ADIPOQ, BMI, AGE, SEX, SMTSISCH, DTHHRDY, SMGEBTCH) 
...
library(broom.mixed)
transverse_lm  <-  lmer(percent ~ ADIPOQ + AGE + SEX+ BMI + SMTSISCH + (1|SUBJID),
                         REML = F,
                         data = transverse_dat)

transverse_lm_2  <-  lmer(percent ~ ADIPOQ + AGE * SEX + BMI + SMTSISCH  + (1|SUBJID),
                         REML = F,
                         data = transverse_dat)
...
for (tiss in tiss_list) {
  file_name <- paste0("input/limma_result_batch_files/limma_",tiss,"_res_sex_batch.csv")
  #Try catch here to read through if file doesn't exist
...
master_lm_sex <- master_lm_sex %>% mutate(all.correct.pval = p.adjust(pvalue, method = "BH"),
...
write.csv(master_lm_sex, file = "output/table_S3_all_sex_results_batch.csv")
```
...
write.csv(supp_table_gene_age, "output/suppl_table_aging_genes_incr_decr_batch.csv")
...
ggplot(hist_df, aes(x = reorder(abrreviated, total_n), y = n, fill = direction)) +
 ...
 # geom_text(aes(label=n)) +
  #theme(axis.text.x = element_text(angle = 90)) +
  scale_fill_manual(name = "Directionality",
                    labels = c("Decreasing", "Increasing"),
                    values = c("#FFC20A", "#0C7BDC")) +
  coord_flip() +
  xlab("Tissue") +
  ylab("Count of significant Ggnes") +
  labs(title = "Genes that change expression with age")
ggsave("output/fig_S3_age_DE_genes_tiss_batch.pdf", width = 7, height = 8)
...
ggplot(hist_sex, aes(x = reorder(abrreviated, total_n), y = n, fill = direction)) +
 ...
 # geom_text(aes(label=n)) +
  #theme(axis.text.x = element_text(angle = 90)) +
  scale_fill_manual(name = "Directionality",
                    labels = c("Female", "Male"),values = c("#0C7BDC", "#FFC20A")) +
  coord_flip() +
  xlab("Tissue") +
  ylab("Count of significant genes") +
  labs(title = "Genes that change expression with sex")

ggsave("output/fig_S4_sex_DE_genes_tiss_batch.pdf", width = 7, height = 8)
...
sex_b_size_plot
ggsave("output/fig_4a_unused_all_sex_beta_batch.pdf", width = 7,
      height = 7)
```
...
sex_p_size_plot <- ggplot(beta_lm_all) +
  geom_point(aes(x = Gene.Symbol, y = tissue, color = direction, size = estimate_abs)) +
  scale_color_manual(labels = c("Higher in Females","Higher in Males"), 
                     values = c("#0C7BDC", "#FFC20A")) +
  scale_y_discrete(limits = rev(ordered_genes$r_names), labels = rev(ordered_genes$abbreviated)) +
  scale_x_discrete(limits = unique(ordered_genes$Gene.Symbol)) +
  geom_tile(aes(x = Gene.Symbol, y = -1,  fill = Category)) +
  scale_fill_manual(name = "Matrix category", values = c(
    "#E66100",
    "#5D3A9B", "#E1BE6A", "#994F00")) +
  geom_point(aes(x = Gene.Symbol, y = top_tiss), shape = 8) +
  #geom_rect(aes(xmin = Gene.Symbol, xmax= lead(Gene.Symbol), ymin = -1, ymax =-.5, fill = division), position = ) +
...
sex_b_size_plot
ggsave("output/fig_4a_all_sex_pvalue_batch.pdf", width = 7,
      height = 7)
```
...
ggsave("output/skin_sex_difference_batch.pdf", width = 7, height = 7.5)
...
ggsave("output/adipose_sex_difference_batch.pdf", width = 6, height = 12)


## Compare sub results to known clusters
subc_only <- adipose_sex %>% filter(tissue == "adipose_subcutaneous")
sub_profiles_genes <- read.csv(file = "../../manual/adipose/adipose_subcutaneous-gene-clusters-high-variance_mat.csv")

fat_ind <- (subc_only$Gene.Symbol %in% sub_profiles_genes$B.1)
sum(fat_ind)
subc_only[fat_ind,]

## Find out how many genes overlap from clusters to DE
fat_ind <- (subc_only$Gene.Symbol %in% sub_profiles_genes$B.1)
sum(fat_ind)
subc_only[fat_ind,]
...
ggsave(filename = "output/all_sex_plot_batch.pdf", width = 13, height = 11)
```
...
write.csv(table_shared_genes, "output/shared_sex_tissues_batch.csv", row.names = T)
```
...
## Get genes with most directionality
for_directionality <- as.data.frame.matrix(dir_mat) %>% mutate(total = negative + positive) 
for_directionality %>% filter(positive == 0) %>% arrange(desc(negative))
for_directionality %>% filter(negative == 0) %>% arrange(desc(positive))
## Least congruent
as.data.frame.matrix(dir_mat) %>% mutate(difference = abs(negative - positive))  %>%
  arrange((difference), desc(positive))
