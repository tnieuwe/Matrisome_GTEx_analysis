...
sub_samp <- filter(samptab, SMTSD == "Adipose - Subcutaneous",
  ...
  select(SAMPID, SMPTHNTS, SMTSD,SMTSISCH) %>% filter(
...
model_visc_fat <- lmer(percent ~ `B.1 Zscore` + SEX +  SMTSISCH +(1|SUBJID), data = test_2_visc)

summary(model_visc_fat)

model_sub_fat <- lmer(percent ~ `B.1 Zscore` + SEX + BMI+ SMTSISCH + (1|SUBJID), data = test_2_sub)

summary(model_sub_fat)


model_all_fat <- lmer(percent ~ `B.1 Zscore` + SEX + SMTSD + SMTSISCH + (1|SUBJID), data = test_2)
...
subc_b_df_2 <- left_join(subc_B_df, indv_phe) %>% left_join(samptab) 
...
subc_sex_model <- lm(`B.1 Zscore` ~ SEX + BMI + SMTSISCH, data =subc_b_df_2)
...
visc_b_df_2 <- left_join(visc_B_df, indv_phe) %>% left_join(samptab)
...
visc_sex_model <- lm(`B.1 Zscore` ~ SEX + BMI + SMTSISCH, data =visc_b_df_2)
...
#library(xlsx)
# write.xlsx(as.data.frame(tidy_sub_model), file="output/suppl_table_fat.xlsx", sheetName="B function of Sex Sub", rowNames=F,overwrite = T)
# write.xlsx(as.data.frame(tidy_visc_model), file="output/suppl_table_fat.xlsx", sheetName="B function of Sex Visc", rowNames=F, append = T, overwrite = T)
# write.xlsx(as.data.frame(sub_coeff_tab), file="output/suppl_table_fat.xlsx", sheetName="Fat percent function of B Sub", rowNames=F, append = T,overwrite = T)
# write.xlsx(as.data.frame(visc_coeff_tab), file="output/suppl_table_fat.xlsx", sheetName="Fat percent function of B Visc", rowNames=F, append = T,overwrite = T)
library(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "B function of Sex ADPSBQ")
addWorksheet(wb, "B function of Sex ADPVSC")
addWorksheet(wb, "Fat percent funct of B ADPSBQ")
addWorksheet(wb, "Fat percent funct of B ADPVSC")
writeData(wb, "B function of Sex ADPSBQ", as.data.frame(tidy_sub_model),
          startRow = 1, startCol = 1)
writeData(wb, "B function of Sex ADPVSC", as.data.frame(tidy_viscc_model),
          startRow = 1, startCol = 1)
writeData(wb, "Fat percent funct of B ADPSBQ", as.data.frame(sub_coeff_tab),
          startRow = 1, startCol = 1)
writeData(wb, "Fat percent funct of B ADPVSC", as.data.frame(visc_coeff_tab),
          startRow = 1, startCol = 1)
saveWorkbook(wb, file = "output/suppl_table_fat.xlsx", overwrite = TRUE)
```
