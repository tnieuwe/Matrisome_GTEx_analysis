...
ggsave("heatmap_rho.pdf", width = 10, height = 8)
...
#   labs(title = "Permuted Correlations and Core Matrisome Correlations", y= "Spearman Rho Correlatiosn") +
#   theme(axis.text.x = element_text(angle = 85, hjust = 1))
# 
# ggsave("rho_permuted.pdf", height = 7, width = 13)
# 
# write.csv(arrange_gene_spear, "data_out/gene_Wang_etal.csv",row.names = F)
# write.csv(arrange_protein_spear, "data_out/protein_Wang_etal.csv",row.names = F)
# 
# log10_gene_spear <- cbind(arrange_gene_spear[,1:2], (log10(as.matrix(arrange_gene_spear[,-1:-2]) + 1)))
# log10_protein_spear <- cbind(arrange_protein_spear[,1:2], (log10(as.matrix(arrange_protein_spear[,-1:-2]) + 1)))
# 
# 
# write.csv(log10_gene_spear, "data_out/gene_Wang_etal_log10.csv",row.names = F)
# write.csv(log10_protein_spear, "data_out/protein_Wang_etal_log10.csv",row.names = F)
```

MArc
```{r}
# arrange_gene_spear
# arrange_protein_spear
# 
# #With 0s
# scatterplot(log10(arrange_protein_spear$Fallopian.tube + 1) ~ log10(arrange_gene_spear$Fallopian.tube + 1), data= join_core)
# 
# core_gene_spear <- arrange_gene_spear %>% filter(Gene.name %in% core_mat$Gene.Symbol)
# 
# core_protein_spear <- arrange_protein_spear %>% filter(Gene.name %in% core_mat$Gene.Symbol)
# 
# scatterplot(log10(core_protein_spear$Fallopian.tube + 1) ~ log10(core_gene_spear$Fallopian.tube + 1), data= join_core)
# 
# #No 0s
# no_0_gene <- arrange_gene_spear %>% filter()


```
Re-do with ecm regulators
```{r}
# #Generalized loop but with permuations
# ecm_dat <- read.csv("data_in/ecm_regulators.csv", stringsAsFactors = F)
# 
# 
# #Rho core and p core hold the rho and p values for core gense only
# rho_core <- NULL
...
# 
# #Core tiss 
# join_core <- join_dat %>% filter(Gene.name %in% ecm_dat$Gene)
# 
# scatterplot(log10(protein + 1) ~ log10(gene + 1), data= join_core)
# 
# test_core <- cor.test(log10(join_core$protein + 1), log10(join_core$gene + 1), method = "spearman" , exact = F)
...
#   theme(axis.text.x = element_text(angle = 85, hjust = 1))
# 
# ggsave("data_out/rho_permuted_ecm_reg.pdf", height = 7, width = 13)
...
Re-do with ecm regulators
```{r}
# #Generalized loop but with samplinh
# ecm_dat <- read.csv("data_in/ecm_regulators.csv", stringsAsFactors = F)
# 
# ecm_core_dat <- c(ecm_dat$Gene, core_mat$Gene.Symbol)
# 
# #Rho core and p core hold the rho and p values for core gense only
# rho_core <- NULL
# p_core <- NULL
...
# 
# #Core tiss 
# join_core <- join_dat %>% filter(Gene.name %in% ecm_core_dat)
# 
# scatterplot(log10(protein + 1) ~ log10(gene + 1), data= join_core)
# 
# test_core <- cor.test(log10(join_core$protein + 1), log10(join_core$gene + 1), method = "spearman" , exact = F)
...
# gene_n <- nrow(join_core)
# 
# gene_prot_counter <- rbind(gene_prot_counter, cbind(cur_tiss, gene_n))
# 
# #Permute genes
# set.seed(1)
# 
...
# 
# ggplot(final_df) + geom_violin(aes(x= Tissue, y = Spearman_Rho, fill = Tissue)) + geom_crossbar(aes(x = Tissue, y = Mat_Core), ymin=0, ymax=0) +
#   labs(title = "Permuted Correlations and ECM regulators and Core Correlations", y= "Spearman Rho Correlatiosn") +
#   theme(axis.text.x = element_text(angle = 85, hjust = 1))
# 
# ggsave("data_out/rho_permuted_ecm_reg_n_core.pdf", height = 7, width = 13)
# 
# gene_prot_counter <- as.data.frame(gene_prot_counter) %>%
#   arrange(desc(gene_n)) %>%
#   rename(Tissue =cur_tiss, pairs = gene_n)
# 
# corr_vs_pair <- left_join(gene_prot_counter, select(final_df, Tissue, Mat_Core) %>% unique()) %>%
#   mutate(pairs = as.numeric(pairs))
# 
# 
# cor.test(corr_vs_pair$pairs, corr_vs_pair$Mat_Core, method = "spearman")
# 
# write.csv(gene_prot_counter, "data_out/gene-protein-corr-pairs.csv")
```
...
ggsave("data_out/rho_permuted_core_reg_all_stdv.pdf", height = 7, width = 13)
...
write.csv(summ_df_table, file = "data_out/collagen_gene_corr.csv")
...
write.csv(summ_df_table, file = "data_out/glycoprotein_gene_corr.csv")
...
ggplot(final_plot) +
  ...
  geom_crossbar(data = summ_df, aes(x = Tissue, y = mean_rho - stdv),
                ymin=0, ymax=0, fatten = 1, color = "red")  +
  theme_classic() +
  labs(title = "Permuted Correlations for Regulatorss Correlations", y= "Spearman Rho Correlatiosn") +
  theme(axis.text.x = element_text(angle = 85, hjust = 1))
...
write.csv(summ_df_table, file = "data_out/regulators_gene_corr.csv")
...
write.csv(summ_df_table, file = "data_out/proteoglycans_gene_corr.csv")
...
write.csv(summarised_cor_reg, "summarized_core_reg_correlations.cvs", row.names = F)
